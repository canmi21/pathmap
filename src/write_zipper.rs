...
pub trait WriteZipper<V>: Zipper {
    ...
    fn join<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: Lattice;
    ...
    fn join_map(&mut self, map: BytesTrieMap<V>) -> bool
    where
        V: Lattice;
    ...
    fn join_into<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: Lattice;
    ...
    /// nonexistent path.
    fn meet<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: Lattice;

    /// Experiment.  GOAT, document this
    fn meet_2<'z, ZA: Zipper<V = V>, ZB: Zipper<V = V>>(&mut self, rz_a: &ZA, rz_b: &ZB) -> bool
    where
        V: Lattice;
    ...
    /// nonexistent path.
    fn subtract<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: PartialDistributiveLattice;
...
impl<V: Clone + Send + Sync> Zipper for WriteZipperTracked<'_, '_, V> {
    ...
    fn fork_read_zipper<'a>(&'a self) -> Self::ReadZipperT<'a> {
        ...
        let rz_core = ReadZipperCore::new_with_node_and_path_internal(
            self.z.focus_stack.top().unwrap().as_tagged(),
            &self.z.key.node_key(),
            None,
            new_root_val,
        );
...
impl<V: Clone + Send + Sync> WriteZipper<V> for WriteZipperTracked<'_, '_, V> {
    ...
    fn meet_2<ZA: Zipper<V = V>, ZB: Zipper<V = V>>(&mut self, rz_a: &ZA, rz_b: &ZB) -> bool
    where
        V: Lattice,
    {
...
impl<V: Clone + Send + Sync> Zipper for WriteZipperCore<'_, '_, V> {
    ...
    fn is_value(&self) -> bool {
        self.focus_stack
            .top()
            .unwrap()
            .node_contains_val(self.key.node_key())
    }
    ...
    fn descend_to<K: AsRef<[u8]>>(&mut self, k: K) -> bool {
        ...
        self.focus_stack
            .top()
            .unwrap()
            .node_contains_partial_key(self.key.node_key())
    }
    ...
    fn make_map(&self) -> Option<BytesTrieMap<Self::V>> {
        self.get_focus()
            .into_option()
            .map(|node| BytesTrieMap::new_with_root(node))
    }
}
...
impl<'a, 'k, V: Clone> zipper_priv::ZipperPriv for WriteZipperCore<'a, 'k, V> {
    ...
    fn get_focus(&self) -> AbstractNodeRef<Self::V> {
        self.focus_stack
            .top()
            .unwrap()
            .get_node_at_key(self.key.node_key())
    }
}
...
impl<'a, 'k, V: Clone + Send + Sync> WriteZipperCore<'a, 'k, V> {
    ...
    pub fn get_value(&self) -> Option<&V> {
        self.focus_stack
            .top()
            .unwrap()
            .node_get_val(self.key.node_key())
    }
    ...
    pub fn get_value_mut(&mut self) -> Option<&mut V> {
        self.focus_stack
            .top_mut()
            .unwrap()
            .node_get_val_mut(self.key.node_key())
    }
        ...
        let created_subnode = self.in_zipper_mut_static_result(
            |node, key| {
                if !node.node_contains_val(key) {
                    node.node_set_val(key, default)
                        .map(|(_old_val, created_subnode)| created_subnode)
                } else {
                    Ok(false)
                }
            },
    ...
    pub fn zipper_head(&mut self) -> ZipperHead<V> {
        if self.at_root() {
            ...
            let focus_stack_ptr: *mut MutCursorRootedVec<
                NonNull<TrieNodeODRc<V>>,
                dyn TrieNode<V> + 'static,
    ...
    pub fn join<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: Lattice,
    {
    ...
    pub fn join_map(&mut self, map: BytesTrieMap<V>) -> bool
    where
        V: Lattice,
    {
    ...
    pub fn join_into<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: Lattice,
    {
        let src = read_zipper.get_focus();
        ...
        match self.get_focus().into_option() {
            Some(mut self_node) => {
                self_node
                    .make_mut()
                    .join_into_dyn(src.into_option().unwrap());
                true
            }
            None => {
                self.graft_internal(src.into_option());
    ...
    pub fn drop_head(&mut self, byte_cnt: usize) -> bool
    where
        V: Lattice,
    {
        match self.get_focus().into_option() {
            Some(mut self_node) => match self_node.make_mut().drop_head_dyn(byte_cnt) {
                Some(new_node) => {
    ...
    pub fn meet<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: Lattice,
    {
        let src = read_zipper.get_focus();
    ...
    pub fn meet_2<ZA: Zipper<V = V>, ZB: Zipper<V = V>>(&mut self, rz_a: &ZA, rz_b: &ZB) -> bool
    where
        V: Lattice,
    {
    ...
    pub fn subtract<Z: Zipper<V = V>>(&mut self, read_zipper: &Z) -> bool
    where
        V: PartialDistributiveLattice,
    {
        let src = read_zipper.get_focus();
            ...
            match focus_node.node_get_child_mut(node_key) {
                Some((consumed_bytes, child_node)) => {
                    if node_key.len() >= consumed_bytes {
                        child_node
                            .make_mut()
                            .node_remove_unmasked_branches(&node_key[consumed_bytes..], mask);
    ...
    fn take_focus(&mut self) -> Option<TrieNodeODRc<V>> {
        ...
        if node_key.len() == 0 {
            ...
            if !self.rooted {
                ...
                self.focus_stack
                    .advance_from_root(|root| Some(unsafe { root.as_mut() }.make_mut()));
            ...
            Err(replacement_node) => {
                ...
                match self.focus_stack.top_mut() {
                    ...
                    None => {
                        if !self.rooted {
                            panic!(
                                "Illegal Operation, cannot modify root of non-rooted WriteZipper"
                            );
                        } else {
                            let stack_root =
                                unsafe { self.focus_stack.root_mut().unwrap().as_mut() };
                            *stack_root = replacement_node;
                            self.focus_stack.advance_from_root(|root| {
                                Some(unsafe { root.as_mut() }.make_mut())
                            });
                        }
                    }
                }
    ...
    fn descend_to_internal(&mut self) {
        ...
        self.focus_stack
            .advance_if_empty(|root| unsafe { root.as_mut() }.make_mut());
...
#[inline]
fn make_parents<V: Clone + Send + Sync>(
    path: &[u8],
    child_node: TrieNodeODRc<V>,
) -> TrieNodeODRc<V> {
    #[cfg(not(feature = "all_dense_nodes"))]
    {
...
mod tests {
    ...
    fn write_zipper_graft_test() {
        ...
        let mut a: BytesTrieMap<i32> = a_keys
            .iter()
            .enumerate()
            .map(|(i, k)| (k, i as i32))
            .collect();
    ...
    fn write_zipper_join_test() {
        ...
        let mut a: BytesTrieMap<u64> = a_keys
            .iter()
            .enumerate()
            .map(|(i, k)| (k, i as u64))
            .collect();
    ...
    fn write_zipper_movement_test() {
        ...
        let mut map: BytesTrieMap<u64> = keys
            .iter()
            .enumerate()
            .map(|(i, k)| (k, i as u64))
            .collect();
    ...
    fn write_zipper_remove_branch_test() {
        ...
        let mut map: BytesTrieMap<i32> = keys
            .iter()
            .enumerate()
            .map(|(i, k)| (k, i as i32))
            .collect();
