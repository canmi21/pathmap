...
impl<'a, V: Clone + Send + Sync> ZipperIteration<'a, V> for ReadZipperTracked<'a, '_, V> {
    ...
    fn descend_first_k_path(&mut self, k: usize) -> bool {
        self.z.descend_first_k_path(k)
    }
    fn to_next_k_path(&mut self, k: usize) -> bool {
        self.z.to_next_k_path(k)
    }
}
...
impl<'a, 'path, V: Clone + Send + Sync> ReadZipperTracked<'a, 'path, V> {
    ...
    pub(crate) fn new_with_node_and_cloned_path(
        root_node: &'a dyn TrieNode<V>,
        path: &[u8],
        root_key_offset: Option<usize>,
        tracker: ZipperTracker<TrackingRead>,
...
impl<'a, 'path, V: Clone + Send + Sync> std::iter::IntoIterator
    for ReadZipperTracked<'a, 'path, V>
{
...
impl<'a, 'path, V: Clone + Send + Sync> ReadZipperUntracked<'a, 'path, V> {
    ...
    pub(crate) fn new_with_node_and_path_internal(
        root_node: TaggedNodeRef<'a, V>,
        path: &'path [u8],
        root_key_offset: Option<usize>,
        root_val: Option<&'a V>,
        tracker: Option<ZipperTracker<TrackingRead>>,
    ) -> Self {
        let core = ReadZipperCore::new_with_node_and_path_internal(
            root_node,
            path,
            root_key_offset,
            root_val,
...
impl<'a, 'path, V: Clone + Send + Sync> std::iter::IntoIterator
    for ReadZipperUntracked<'a, 'path, V>
{
    type Item = (Vec<u8>, &'a V);
    type IntoIter = ReadZipperIter<'a, 'path, V>;
...
impl<V> Clone for ReadZipperCore<'_, '_, V>
where
    V: Clone,
{
    fn clone(&self) -> Self {
...
impl<V: Clone + Send + Sync> Zipper for ReadZipperCore<'_, '_, V> {
    ...
    fn descend_to<K: AsRef<[u8]>>(&mut self, k: K) -> bool {
        ...
        while let Some((consumed_byte_cnt, next_node)) = self.focus_node.node_get_child(key) {
            ...
            self.ancestors
                .push((self.focus_node.clone(), self.focus_iter_token, key_start));
    ...
    fn fork_read_zipper<'a>(&'a self) -> Self::ReadZipperT<'a> {
        ...
        Self::ReadZipperT::new_with_node_and_path_internal(
            self.focus_node.clone(),
            new_root_path,
            new_root_key_offset,
            new_root_val,
...
impl<'a, V: Clone + Send + Sync> ReadOnlyZipper<'a, V> for ReadZipperCore<'a, '_, V> {
    fn get_value(&self) -> Option<&'a V> {
        let key = self.node_key();
        if key.len() > 0 {
            ...
            if new_tok != NODE_ITER_FINISHED {
                ...
                if let Some((focus_node, iter_tok, prefix_offset)) = self.ancestors.pop() {
                    ...
                    let new_len = self
                        .origin_path
                        .len()
                        .max(self.root_key_offset.unwrap_or(0));
                    self.focus_iter_token = NODE_ITER_INVALID;
    ...
    fn descend_first_k_path(&mut self, mut k: usize) -> bool {
        ...
        if full_key.len() > node_key_len {
            if full_key.len() >= node_key_len + k {
                self.prefix_buf
                    .extend(&full_key[node_key_len..node_key_len + k]);
                if full_key.len() == node_key_len + k {
                    self.regularize();
                }
                return true;
...
fn node_along_path<'a, 'path, V>(
    root_node: &'a dyn TrieNode<V>,
    path: &'path [u8],
) -> (&'a dyn TrieNode<V>, &'path [u8], Option<&'a V>) {
    let mut key = path;
...
impl<'a, 'path, V: Clone + Send + Sync> ReadZipperCore<'a, 'path, V> {
    ...
    pub(crate) fn new_with_node_and_path(
        root_node: &'a dyn TrieNode<V>,
        path: &'path [u8],
        mut root_key_offset: Option<usize>,
    ) -> Self {
            ...
            if new_tok != NODE_ITER_FINISHED {
                ...
                if key_start < base_idx {
                    ...
                    if fixed_len > key_bytes.len()
                        || &key_bytes[..fixed_len] != &self.prefix_buf[key_start..]
                    {
                        self.prefix_buf.truncate(base_idx);
                        return false;
                    }
...
mod tests {
    ...
    fn zipper_basic_test() {
        ...
        assert_eq!(
            rz.child_mask().byte_mask_iter().collect::<Vec<_>>(),
            vec![b'n']
        ); // both follow-ups romane and romanus have n following a
        assert!(rz.to_sibling(true)); // focus = romu  (u is the third byte)
        assert_in_list(rz.path(), &[b"roma", b"romu"]);
        assert_eq!(
            rz.child_mask().byte_mask_iter().collect::<Vec<_>>(),
            vec![b'l']
        ); // and romu is followed by lus
    ...
    fn zipper_with_starting_key() {
        ...
        let root_key = b"ro";
        let mut zipper = ReadZipperCore::new_with_node_and_path(
            btm.root().borrow(),
            root_key,
            Some(root_key.len()),
        );
    ...
    fn zipper_child_mask_test() {
        let keys = vec![vec![8, 194, 1, 45, 194, 1], vec![34, 193]];
        let map: BytesTrieMap<u64> = keys
            .iter()
            .enumerate()
            .map(|(i, k)| (k, i as u64))
            .collect();
    ...
    fn path_concat_test() {
        ...
        let family: BytesTrieMap<i32> = exprs
            .iter()
            .enumerate()
            .map(|(i, k)| (k, i as i32))
            .collect();
    ...
    fn full_path_test() {
        ...
        assert_eq!(
            std::str::from_utf8(zipper.origin_path().unwrap()).unwrap(),
            "roma"
        );
    ...
    fn k_path_test3() {
        let keys = [
            ":1a1A", ":1a1B", ":1a1C", ":1b1A", ":1b1B", ":1b1C", ":1c1A",
        ];
        let map: BytesTrieMap<u64> = keys
            .into_iter()
            .enumerate()
    ...
    fn k_path_test4() {
        let keys = vec![
            ...
            vec![
                180, 216, 189, 14, 82, 14, 170, 195, 196, 42, 177, 144, 153, 156, 140, 109, 93, 78,
                157,
            ],
            ...
            vec![
                34, 132, 138, 222, 250, 162, 231, 68, 142, 162, 152, 172, 244, 102, 179, 111, 161,
                95,
            ],
            vec![
            ...
            vec![
                89, 248, 169, 97, 245, 98, 230, 53, 114, 198, 227, 148, 22, 127, 198, 153, 238, 59,
                223,
            ],
            vec![
            ...
            vec![
                115, 201, 114, 124, 135, 246, 93, 230, 210, 164, 213, 254, 108, 181, 77, 19, 103,
                166,
            ],
            ...
            vec![
                192, 215, 69, 171, 218, 187, 202, 120, 92, 33, 14, 77, 34, 46, 40, 93, 135, 117,
                152,
            ],
        ];
    ...
    fn zipper_byte_iter_test1() {
        let keys = ["ABCDEFGHIJKLMNOPQRSTUVWXYZ", "ab"];
        let map: BytesTrieMap<u64> = keys
            .into_iter()
            .enumerate()
            .map(|(i, k)| (k, i as u64))
